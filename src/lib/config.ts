...
const envSchema = z.object({
  ...
  EA_BUILDERTREND_BASE_URL: z.string().url().default("https://api.buildertrend.com/v1"),
...
export const validateConfig = () => {
  ...
  if (features.googleWorkspace && (!externalServices.google.clientId || !externalServices.google.clientSecret)) {
    errors.push("Google Workspace integration enabled but missing OAuth credentials");
  }
  ...
  if (storageConfig.type === "s3" && (!storageConfig.aws.accessKeyId || !storageConfig.aws.secretAccessKey || !storageConfig.aws.bucket)) {
    errors.push("S3 storage configured but missing AWS credentials or bucket name");
  }
